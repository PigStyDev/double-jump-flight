...
const flightEnabled = new Map<string, boolean>();
const lastJumpTime = new Map<string, number>();
const groundedSince = new Map<string, number>();

const UP_FORCE = 0.06;
const FORWARD_FORCE = 0.03;
const GROUNDED_EXIT_MS = 600;

function lookVector(player: Player) {
...
system.runInterval(() => {
  for (const player of world.getPlayers()) {
    if (!(flightEnabled.get(player.id) ?? false)) continue;
